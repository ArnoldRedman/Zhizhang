...
export const draftAcceptanceIssues = (content: string, target: number, review?: ReviewForAcceptance): string[] => {
  ...
  if (!review?.perspectives?.length || review?.reviewFailures?.length) {
    const failures = review?.reviewFailures?.filter(Boolean) || [];
    issues.push(failures.length ? `审查未完成：${failures.join('；')}` : '审查未完成');
  }
