...
## 核心目标
确保本章与上一章在人物、情节、情绪、设定四个维度无缝衔接，并在衔接之后把故事推进一个新的节点。上一章结尾是本章开头的最高优先级资料，不是本章的内容。

## 承接步骤
1. 读取上一章最后 800 字和该章记忆中的摘要、章末钩子。
...
7. 承接只占开头一到三段。写出承接之后必须离开这个场景：把上一章留下的事件收束掉，或者用一到两段过渡把时间、地点推进到本章计划的新节点；本章主体只能是这个新节点。

## 输出约束
先在内部形成“位置、情绪、未解决事件、道具线索、时间线、伏笔、章节钩子”七项承接清单，再写正文。正文第一段自然承接，不要复述清单或解释写作过程。

## 禁止
上一章在冲突中，本章无过渡直接跳到平静日常；忽略上一章钩子和伏笔；人物从 A 地愤怒地跳到 B 地平静而没有原因。把上一章最后的动作、对峙或对话重新详细描写一遍当作本章主体；连续多章停留在同一地点、同一件事里不推进。上一章已经完成、已经交付、已经揭晓的事不得重新发生或重新揭晓一次。`,
  },
  {
