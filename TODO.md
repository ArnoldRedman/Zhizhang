...
- [ ] **章纲勾选状态切章后不清空，写新章时带着上一章的章纲**
  ...
  - 改法：运行章节智能体时只用绑定当前章（chapterId 或章号匹配）的章纲；切章自动重置勾选，勾选框只做补充。
- [ ] **总纲从不进入章节智能体和章纲智能体**
  ...
  - 改法：总纲以"结构骨架 + 当前阶段正文"的形式进入两个智能体，配"不得提前兑现后续节点"的约束；防剧透靠指令，不靠整份屏蔽。
- [ ] **章节智能体只拿到上一章一章正文和一条记忆**
...
- [ ] **章纲生成只看上一章正文，模板每章强制"新危机"**
  ...
  - 改法：章纲输入加总纲当前阶段与最近几章摘要；模板加"本章推进的总纲节点""与前文的区别"两栏。
- [ ] **项目 Agent 路径与界面路径的入参不一致**
...
- [ ] **质量门禁只在打 tag 时运行**
  ...
  - 改法：加 `typecheck:desktop` 并纳入根 typecheck；tsconfig 显式 strict；本地构建脚本先跑前端检查再构建；新增 push/PR 触发的质量工作流。
